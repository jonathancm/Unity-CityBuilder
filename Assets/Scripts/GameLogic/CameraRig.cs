...
public class CameraRig : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        initialZoomDistance = _camera.transform.localPosition;
    ...
    private void Update()
    {
        ...
        // Chase Zoom Goal
        //
        bool zoomGoalReached = Vector3.Distance(_camera.transform.localPosition, targetZoomDistance) < 0.1f;
        if (!zoomGoalReached)
            _camera.transform.localPosition = Vector3.Lerp(_camera.transform.localPosition, targetZoomDistance, cameraSettings.zoomSpeed * Time.deltaTime);
        ...
        if (!positionGoalReached)
            transform.position = Vector3.Lerp(transform.position, targetPosition, cameraSettings.panSpeed * _camera.transform.localPosition.magnitude * Time.deltaTime);
    ...
    public void Zoom(float zoomAxis)
    {
        ...
        float newTarget = _camera.transform.localPosition.y - (zoomAxis * cameraSettings.zoomSpeed);
